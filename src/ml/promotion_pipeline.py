...
class PromotionPipeline:
    ...
    def generate_promotion_dashboard(self) -> str:
        """Generate HTML promotion dashboard."""
        
        report = self.generate_promotion_report()
        
        # Create stage progress bars
        stages = [stage.value for stage in PromotionStage]
        current_stage_idx = stages.index(report['current_status']['current_stage'])
        ...
        <html>
        <head>
            <title>Promotion Pipeline Dashboard</title>
            <style>
                body {{ font-family: Arial, sans-serif; margin: 20px; background-color: #f5f5f5; }}
                .header {{ background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); 
                          color: white; padding: 20px; border-radius: 10px; margin-bottom: 20px; }}
                .pipeline {{ display: flex; justify-content: space-between; margin: 20px 0; }}
                .stage {{ flex: 1; margin: 0 10px; padding: 15px; border-radius: 8px; text-align: center; }}
                .stage.completed {{ background-color: #d4edda; border: 2px solid #28a745; }}
                .stage.current {{ background-color: #fff3cd; border: 2px solid #ffc107; }}
                .stage.pending {{ background-color: #f8d7da; border: 2px solid #dc3545; }}
        ...
        <body>
            <div class="header">
                <h1>🚀 Promotion Pipeline Dashboard</h1>
                <p>Automated promotion gates with quality assurance</p>
            </div>
            ...
            <div class="metrics-grid">
                <div class="metric-card">
                    <div class="metric-value">{report['current_status']['current_stage'].replace('_', ' ').title()}</div>
                    <div class="metric-label">Current Stage</div>
                </div>
                
                <div class="metric-card">
                    <div class="metric-value">{report['current_status']['total_evaluations']}</div>
                    <div class="metric-label">Total Evaluations</div>
                </div>
                
                <div class="metric-card">
                    <div class="metric-value">{report['current_status']['recent_evaluations']}</div>
                    <div class="metric-label">Recent Evaluations</div>
                </div>
                
                <div class="metric-card">
                    <div class="metric-value">{len(report['recent_results'])}</div>
                    <div class="metric-label">Recent Results</div>
                </div>
            </div>
            
            <h3>Recent Promotion Results</h3>
            <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
                ...
                <tbody>
                    {''.join([f"""
                        <tr>
                            <td style="border: 1px solid #ddd; padding: 8px;">{result['gate_name']}</td>
                            <td style="border: 1px solid #ddd; padding: 8px;">{result['stage']}</td>
                            <td style="border: 1px solid #ddd; padding: 8px;">{result['status']}</td>
                            <td style="border: 1px solid #ddd; padding: 8px;">{result['score']:.2f}</td>
                            <td style="border: 1px solid #ddd; padding: 8px;">{result['evaluation_date'][:10]}</td>
                        </tr>
        ...
        # Save HTML dashboard
        html_path = Path("reports/ml/promotion_dashboard.html")
        html_path.parent.mkdir(parents=True, exist_ok=True)
        
        with open(html_path, 'w', encoding='utf-8') as f:
            f.write(html)
        
        logger.info(f"Promotion dashboard saved to {html_path}")
        return html
...
def main():
    ...
    pipeline.generate_promotion_dashboard()
    ...
    print(f"   Dashboard: reports/ml/promotion_dashboard.html")
